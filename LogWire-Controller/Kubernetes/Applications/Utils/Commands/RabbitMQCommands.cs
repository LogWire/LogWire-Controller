...
namespace LogWire.Controller.Kubernetes.Applications.Utils.Commands
{
    public class RabbitMQCommands
    {
        ...
        private static string CCommand = "-c";
        private static string bash = "/bin/sh";

        private static string Bootstrap1 = "set -e\n" +
                                           "cp /configmap/* /etc/rabbitmq\n" +
                                           "echo \"${RABBITMQ_ERLANG_COOKIE}\" > /var/lib/rabbitmq/.erlang.cookie";

        private static string Probe = @"wget -O - -q --header ""Authorization: Basic `echo -n \""$RABBIT_MANAGEMENT_USER:$RABBIT_MANAGEMENT_PASSWORD\"" | base64`"" http://localhost:15672/api/healthchecks/node | grep -qF ""{\""status\"":\""ok\""}""";
